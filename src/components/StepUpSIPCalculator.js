...
export default function StepUpSIPCalculator({ schemeCode, navHistory }) {
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        setError(data.error || 'Failed to calculate Step-up SIP');
      } else {
        setResult(data);
      }
    } catch (err) {
      setError('Failed to calculate Step-up SIP. Please try again.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <Grid item xs={6}>
              ...
              >
                ...
                <Typography variant="h6" sx={{ fontWeight: 700, color: '#88C785' }}>
                  ₹{(result.currentValue - result.totalInvested).toLocaleString()}
                </Typography>
              </Box>
            </Grid>
          </Grid>
          ...
          {result.chartData && result.chartData.length > 0 && (
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 600 }}>
                Investment Growth with Step-up
              </Typography>
