...
export default function StepUpSWPCalculator({ schemeCode, navHistory }) {
    ...
    try {
      const response = await fetch(`/api/scheme/${schemeCode}/stepup-swp`, {
        ...
        body: JSON.stringify({
          ...
          stepUpPercentage: parseFloat(formData.stepUpPercentage),
      ...
      if (!response.ok || data.error) {
        setError(data.error || 'Failed to calculate Step-up SWP');
      } else {
        setResult(data);
      }
    } catch (err) {
      setError('Failed to calculate Step-up SWP. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            helperText="Annual increase in withdrawal"
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
        ...
        <Grid item xs={12}>
          <Button
            fullWidth
            variant="contained"
            startIcon={loading ? <CircularProgress size={20} color="inherit" /> : <TrendingDownIcon />}
            ...
            <Grid item xs={6}>
              ...
              >
                ...
                <Typography variant="h6" sx={{ fontWeight: 700, color: '#88C785' }}>
                  ₹{(result.totalWithdrawn + result.finalBalance - formData.initialAmount).toLocaleString()}
                </Typography>
              </Box>
            </Grid>
          </Grid>
          ...
          {result.chartData && result.chartData.length > 0 && (
            <Box>
              <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 600 }}>
                Balance Over Time with Step-up Withdrawals
              </Typography>
